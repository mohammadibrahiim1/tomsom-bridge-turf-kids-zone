...
import { Slot } from '../features/slot-management/slotTypes/slot.types';
import { useGetAllSlotsQuery } from '../features/slot-management/service/slotApi/slotApi';




export interface TurfBookingFlowProps {
...
/** Map backend English status → Bengali status used in UI */
const mapStatusToBn = (status?: string): TimeSlot['currentStatus'] => {
...
/** Convert backend Slot → frontend TimeSlot shape */
const mapSlotToTimeSlot = (slot: Slot): TimeSlot & { currentStatus: TimeSlot['currentStatus'] } => {
  // Try common field names so the code survives backend changes
  const id = (slot as any).id || slot.slotId || '';
  const startTime =
    (slot as any).startTime ||
    (slot as any).displayTime?.split(' - ')[0]?.trim() ||
    (slot as any).time?.split('-')[0]?.trim() ||
    '—';
  const endTime =
    (slot as any).endTime ||
    (slot as any).displayTime?.split(' - ')[1]?.trim() ||
    (slot as any).time?.split('-')[1]?.trim() ||
    '—';
  const regularPrice =
    (slot as any).regularPrice ??
    (slot as any).totalPrice ??
    (slot as any).price ??
    0;
  const slotType =
    (slot as any).slotType ||
    (slot as any).sportType ||
    (slot as any).packageName ||
    'টার্ফ';

  return {
    // Keep the original slot fields while ensuring normalized values take precedence.
    ...slot,
    regularPrice: Number(regularPrice) || 0,
    slotType,
    playDuration: (slot as any).playDuration || '৫৫ মিনিট খেলা + ৫ মিনিট ইন/আউট',
    currentStatus: mapStatusToBn(slot.status),
  } as any;
};
...
export const TurfBookingFlow: React.FC<TurfBookingFlowProps> = ({
  ...
  // Auto-select initialSlotId when data arrives
  useEffect(() => {
  ...
  // Client-side filter by slot type
      ...
      {confirmedBooking ? (
        ...
        <div className="space-y-8">
          <div className="text-center space-y-2">
            <h1 className="text-3xl sm:text-4xl font-black text-gray-950">অনলাইন টার্ফ বুকিং</h1>
            <p className="text-sm text-gray-600">
              তারিখ ও স্লট নির্বাচন করে সহজে আপনার ম্যাচ বুক করুন। প্রতিটি স্লট ৬০ মিনিট (৫৫ মিনিট খেলা + ৫ মিনিট
              ইন/আউট)।
            </p>
          </div>
              ...
              <div className="bg-white p-6 rounded-md shadow-sm border border-gray-200 space-y-4">
                        ...
                        >
                          ...
                          <div
                            className={`mt-2 pt-1 border-t w-full text-sm font-bold ${
                              isSelected ? 'border-red-400 text-white' : 'border-gray-100 text-red-600'
                            }`}
                          >
                ...
                <div className="bg-red-50 border border-red-200 p-3 rounded-md text-xs text-red-900 flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    প্রতিটি স্লট ৬০ মিনিটের। এর মধ্যে ৫৫ মিনিট খেলা এবং ৫ মিনিট খেলোয়াড়দের ইন/আউট ও মাঠ প্রস্তুত করার
                    সময়।
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white p-6 rounded-md shadow-sm border border-gray-200 space-y-4">
                  ...
                  <div className="flex items-start space-x-2 pt-1">
                    ...
                    >
                      ...
                      <span className="font-bold text-red-600 underline">নিয়ম ও শর্তাবলি</span> এবং
                      বাতিল নীতিমালা (কমপক্ষে ৩ দিন পূর্বে অবহিতকরণ) মেনে নিচ্ছি।
                    </label>
                  </div>
